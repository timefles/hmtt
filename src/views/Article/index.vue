  ...
  <div class="article-container">
    ...
    <van-popup v-model="addCommentShow" position="bottom">
      <AddComment
        ...
        "
      ></AddComment>
      <!-- v-if作用:每次关闭组件销毁 每次打开组件重新创建mounted就会再一次执行 -->
    </van-popup>
...
export default {
  ...
  async created () {
    ...
    // 在这一块的时候loading关闭了，贝面就显示正文,，但是必须还要写在nextTick
    // 因为数据更新视图不能立即更新写在nextTick回调中可以因为nextTick回调就是
    // 在视图更新完成才会触发
    this.$nextTick(() => {
      // 获取正文中所有图片
      const arr = document.querySelectorAll('.article-content img')
      if (arr.length === 0) return // 这行是防御性代码 更加安全 不写后面可能会报错
      const arr1 = []
